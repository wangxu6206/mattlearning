...
<cfcomponent output="false" hint="Handles a single CSS rule.">
	...
	<cfset VARIABLES.Instance.CSS[ "height" ] = "" />
	...
	<cffunction
		...
		returntype=any
		output="false"
		hint="Returns the Java AWT color object.">
		...
		<cfif arguments.colorValue.startsWith('rgb')>
			<cfset Integer = createObject("java","java.lang.Integer")>
			<cfset rgbValue = Replace(arguments.colorValue, "rgb(", "", "All")>
			<cfset rgbValue = Replace(rgbValue, ")", "",  "All")>
			<cfif listLen(rgbValue) == 3>
				<cfset r = Integer.valueOf(trim(listGetAt(rgbValue, 1)))>
				<cfset g = Integer.valueOf(trim(listGetAt(rgbValue, 2)))>
				<cfset b = Integer.valueOf(trim(listGetAt(rgbValue, 3)))>
			</cfif>
			<cfset returnColor = createObject("java","java.awt.Color").init(r, g, b)>
		</cfif>

		<cfif arguments.colorValue.startsWith('##')>
			<cfset Color = createObject("java","java.awt.Color")>
			<cfset returnColor = Color.decode(arguments.colorValue)>
		</cfif>
